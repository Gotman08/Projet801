...
\usebackgroundtemplate{%
  \begin{tikzpicture}[remember picture,overlay]
    \node[opacity=0.12, anchor=south] at ([shift={(0,-1cm)}]current page.south) {%
...
\node[anchor=north west] at ([shift={(2.6cm,-0.15cm)}]current page.north west) {%
  \includegraphics[height=0.75cm]{assets/logo.jpg}%
};
...
\hfill
\raisebox{-0.8cm}{\includegraphics[width=3.2cm]{results/figures/results/gallery/skyline_seed42.png}}
...
\setbeamertemplate{subsection in toc}{\hspace{1em}{\color{burgundy!80!black}\scriptsize\inserttocsectionnumber.\inserttocsubsectionnumber}~~{\small\color{burgundy!60!black}\inserttocsubsection}\par}
\begin{columns}[T]
...
\centering
\vspace{-0.5cm}
{\color{burgundy}\rule{2pt}{0.78\textheight}}
\end{column}
\begin{column}{0.47\textwidth}
...
\subsection{Sample $\to$ output}
\subsection{Compatibilité par overlap}
...
% ============================================================
\section{Architecture et structures}
\subsection{Trois libs CMake}
\subsection{Wave NUMA-friendly}

\begin{frame}{Architecture : trois libs CMake}
\begin{center}
\begin{tikzpicture}[
    lib/.style={draw, rounded corners=2pt, fill=burgundy!15, minimum height=0.65cm, inner sep=4pt, font=\sffamily\small},
    backend/.style={draw, rounded corners=2pt, fill=gold!15, minimum height=0.65cm, inner sep=4pt, font=\sffamily\small},
    bin/.style={draw, fill=burgundy!5, rounded corners=2pt, minimum height=0.5cm, inner sep=2pt, font=\sffamily\scriptsize},
    arrow/.style={->, >=stealth, burgundy, thick},
    scale=0.95, transform shape,
]
\node[lib] (core) at (0,0) {wfc\_core};
\node[backend] (omp) at (-3.6,-1.7) {wfc\_omp\_lib};
\node[backend] (kokkos) at (3.6,-1.7) {wfc\_kokkos\_lib};
\node[bin] (s1) at (-6,-3.0) {wfc\_serial};
\node[bin] (s2) at (-3.6,-3.0) {wfc\_omp};
\node[bin] (s3) at (3.6,-3.0) {wfc\_kokkos};
\node[bin] (s4) at (6,-3.0) {wfc\_dungeon};
\draw[arrow] (core) -- (omp);
...
\footnotesize
    ...
    \item Pattern \textbf{Template Method} : SolverBase tient retry/stats/dispatch, chaque backend fournit \texttt{pick\_cell()} et \texttt{propagate()}.
\end{itemize}
\end{frame}
...
% ============================================================
\section{Profilage et OpenMP \texttt{task}}
\subsection{Profilage série}
\subsection{Min-entropie : \texttt{omp task} chunked}
\subsection{Propagation : single parallel region}
\subsection{Atomic relaxed-load}
\subsection{A/B des optimisations}
...
\begin{itemize}
\footnotesize
    \item Effort principal sur la \emph{sélection}, pas la propagation.
    \item Les deux étapes sont parallélisées avec l'API \textbf{\texttt{\#pragma omp task}} explicite (sujet CHPS0801).
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
% ============================================================
\begin{frame}{Bilan des optimisations en gain temps}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\includegraphics[width=\textwidth]{results/figures/optim/frontier_threshold_compare.png}\\
{\scriptsize A/B frontier-threshold sur Romeo (job SLURM 544356)}
\end{column}
\begin{column}{0.46\textwidth}
\textbf{Cumul des optims gardées}
\begin{block}{binary\_L11 256$\times$256, Romeo}
\centering
\footnotesize
\begin{tabular}{lrr}
\toprule
\textbf{Configuration} & \textbf{Temps} & \textbf{Speedup} \\
\midrule
Serial (1t) & 61.4\,s & 1.0$\times$ \\
\midrule
OMP 8t baseline & 9.13\,s & 6.7$\times$ \\
OMP 16t baseline & 7.55\,s & 8.13$\times$ \\
+ relaxed-load skip & 7.5\,s & 8.18$\times$ \\
+ frontier-threshold & 7.46\,s & \textbf{8.23$\times$} \\
+ LTO & 7.21\,s & 8.52$\times$ \\
\bottomrule
\end{tabular}
\end{block}

\vspace{0.2cm}
{\footnotesize Chaque optim ajoute un gain mesuré indépendamment ; cumul $\sim 8.5\times$ vs serial.}
\end{column}
\end{columns}
\end{frame}

% ============================================================
\section{Kokkos : portabilité CPU + GPU}
\subsection{Architecture \texttt{View} + \texttt{parallel\_for}}
\subsection{Contraintes GPU et atomics}
...
\end{column}
\begin{column}{0.42\textwidth}
\textbf{API Kokkos utilisée}
\begin{itemize}
\footnotesize
    \item \texttt{Kokkos::View<u64*, MemSpace>} : wave + règles
    \item \texttt{Kokkos::parallel\_for} : dispatch propagation
    \item \texttt{KOKKOS\_INLINE\_FUNCTION} : functor portable host/device
    \item \texttt{Kokkos::atomic\_*} : load, fetch\_and, compare\_exchange
\end{itemize}
...
\footnotesize
    ...
    \item GPU : \texttt{Kokkos::deep\_copy} host $\leftrightarrow$ device
    \item 0 overhead sur build CPU, 1 source pour OMP/CUDA
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\subsection{Parallel-attempts, D4, backtrack}
\subsection{Démo Unreal Engine 5}
...
\subsection{Déterminisme bit-à-bit}

\begin{frame}{Tests et validation}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\textbf{15 suites, 24\,980 invariants}
\begin{block}{Tests par couche}
\centering
\footnotesize
\begin{tabular}{lr}
\toprule
\textbf{Suite} & \textbf{Checks} \\
\midrule
\texttt{test\_bitset} & 310 \\
\texttt{test\_grid} + \texttt{test\_grid\_io} & 848 \\
\texttt{test\_overlap} & 4\,898 \\
\texttt{test\_wave} & 8\,594 \\
\texttt{test\_solver\_common} & 9\,060 \\
\texttt{test\_edge\_cases} & 404 \\
\texttt{test\_solver\_omp} + \texttt{kokkos} & 58 \\
\midrule
...
\begin{itemize}
\footnotesize
    \item \textbf{99\% line coverage} (gcovr 8.6, 925 / 933 lignes)
    \item Build dédié \texttt{Debug + --coverage}
    \item ThreadSanitizer (\texttt{USE\_TSAN=ON}) : 0 race
\end{itemize}

\vspace{0.2cm}
\textbf{Tests d'invariants > tests d'output}
\begin{itemize}
\footnotesize
    \item Sortie n'utilise que tuiles du sample
    \item Symétrie des règles overlap
    \item $\text{rotated}_{90}^4 = \text{identité}$
    \item Bit-identité serial = OMP = Kokkos pour $\{1, 2, 4, 8\}$ threads
\end{itemize}

\vspace{0.2cm}
\begin{exampleblock}{Vérification déterminisme}
\centering
\footnotesize
\texttt{diff out\_serial.txt out\_omp.txt} \\
$\to$ identique
\end{exampleblock}
\end{column}
\end{columns}
...
% ============================================================
\section{Résultats}
\subsection{Protocole SLURM Romeo}
\subsection{Speedup et scaling}
\subsection{Heatmap et Amdahl}
\subsection{Galerie WFC}
...
% ============================================================
\begin{frame}{Galerie : input $\to$ output}
\begin{columns}[c]
\begin{column}{0.5\textwidth}
\centering
{\scriptsize \textbf{binary\_5x5} (5$\times$5, $N=2$, 11 tuiles)}\\[2pt]
\includegraphics[height=1.7cm]{results/figures/results/inputs/binary_5x5_input.png}
\hspace{0.3cm}$\to$\hspace{0.3cm}
\includegraphics[height=1.7cm]{results/figures/results/binary_5x5_64x64_N2_seed42.png}\\[1pt]
{\tiny input \hspace{1.2cm} output 64$\times$64}
\end{column}
\begin{column}{0.5\textwidth}
\centering
{\scriptsize \textbf{terrain} (14$\times$10, $N=2$, 33 tuiles)}\\[2pt]
\includegraphics[height=1.7cm]{results/figures/results/inputs/multivalue_terrain_input.png}
\hspace{0.3cm}$\to$\hspace{0.3cm}
\includegraphics[height=1.7cm]{results/figures/results/multivalue_terrain_128x128_N2_seed11.png}\\[1pt]
{\tiny input \hspace{1.2cm} output 128$\times$128}
\end{column}
\end{columns}
...
\begin{column}{0.5\textwidth}
\centering
{\scriptsize \textbf{skyline} (16$\times$14, $N=3$, 79 tuiles)}\\[2pt]
\includegraphics[height=1.7cm]{results/figures/results/inputs/skyline_input.png}
\hspace{0.3cm}$\to$\hspace{0.3cm}
\includegraphics[height=1.7cm]{results/figures/results/gallery/skyline_seed42.png}\\[1pt]
{\tiny input \hspace{1.2cm} output (seed=42)}
\end{column}
\begin{column}{0.5\textwidth}
\centering
{\scriptsize \textbf{plant} (16$\times$15, $N=3$, 94 tuiles)}\\[2pt]
\includegraphics[height=1.7cm]{results/figures/results/inputs/plant_input.png}
\hspace{0.3cm}$\to$\hspace{0.3cm}
\includegraphics[height=1.7cm]{results/figures/results/gallery/plant_seed42.png}\\[1pt]
{\tiny input \hspace{1.2cm} output (seed=42)}
\end{column}
\end{columns}

\vspace{0.15cm}
{\footnotesize Tous obtenus avec \texttt{wfc\_serial}, bit-identiques en OMP et Kokkos pour le même seed.}
\end{frame}
...
\vfill
\begin{center}
{\fontsize{28}{32}\selectfont\color{burgundy}\bfseries Merci pour votre attention}\\[0.4cm]
{\Large\color{gold}Des questions ?}\\[0.8cm]
{\itshape Wave Function Collapse -- CHPS0801}\\[0.2cm]
{\small Romain Despoullains \& Nicolas Marano}
\end{center}
\vfill
\begin{center}
\includegraphics[height=1.4cm]{assets/logo-urca.png}\hspace{1.5cm}%
\includegraphics[height=1.4cm]{assets/logo.jpg}
\end{center}
\vfill
\end{frame}
\endgroup
