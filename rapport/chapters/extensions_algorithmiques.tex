...
Voir \texttt{schemas/ue5\_pipeline.md} pour le prompt de génération.\\
\texttt{wfc\_dungeon} CLI → JSON (multi-étages, stairs, walkable cells) → \\
plugin UE5 → \texttt{ADungeonGenerator} AActor → \texttt{StaticMeshComponent} par cellule.
}}
\caption{Pipeline asset → JSON → spawn de meshes UE5.}
...
JSON v2 (multi-étages) ajoute un wrapper \texttt{"levels": [...]} et inclut un placement automatique d'escaliers entre étages adjacents (cellule \texttt{walkable} commune à deux niveaux successifs).

\subsection{Connectivité par flood-fill BFS}

Avant d'émettre le JSON, \texttt{wfc\_dungeon} vérifie qu'à chaque étage les cellules \textit{walkable} forment une composante connexe (par flood-fill BFS). Si plusieurs composantes sont détectées, l'algorithme re-roll avec un nouveau seed (jusqu'à \texttt{--connectivity-attempts} fois). Garantit qu'un joueur peut traverser tout le niveau.

\subsection{Plugin UE 5.7}

Le plugin \texttt{ue5\_plugin/WFCDungeon/} contient :
\begin{itemize}
    \item \texttt{UTileMappingDataAsset} : DataAsset qui mappe chaque \texttt{tile\_id} vers un struct \{floor, wall, corner, door StaticMesh*\} + flags \texttt{bIsSolid}/\texttt{bIsDoor} + offset/scale.
    \item \texttt{ADungeonGenerator} : AActor avec fonctions \textit{CallInEditor} \textit{Generate From Json} / \textit{Clear Generated}. Parse le JSON, walke les cells, spawne les \texttt{StaticMeshComponent} avec yaw cardinal (-90°N, +90°S, 0°E, 180°W).
\end{itemize}

\subsection{Build opt-in}

\texttt{-DBUILD\_DUNGEON=ON} active la cible. Sans, la cible n'est pas générée du tout (vérifié : \texttt{ninja: no work to do} après reconfigure). Aucun impact sur le benchmark, les ctests, ou les solveurs parallèles.
