...
\epigraph{\textit{``Extensions should compose. Extensions should be optional. Extensions should not slow the default path.''}}{--- Ingénierie logicielle pragmatique}
...
\begin{figure}[H]
\centering
\schemaImage{Pipeline wfc-dungeon vers UE5 plugin}
...
\begin{itemize}
    \item \texttt{UTileMappingDataAsset} : DataAsset qui mappe chaque \texttt{tile\_id} vers un struct \{floor, wall, corner, door StaticMesh* + \texttt{*MeshVariants} arrays\} + flags \texttt{bIsSolid}/\texttt{bIsDoor} + offset/scale. Les arrays de variants permettent de varier les meshes par tile id (random pick avec fallback single-mesh).
    \item \texttt{ADungeonGenerator} : AActor qui parse le JSON, walke les cells, et spawne les \texttt{StaticMeshComponent} avec yaw cardinal (-90°N, +90°S, 0°E, 180°W).
\end{itemize}

\subsection{Gameplay actors et navmesh}

Au-delà du spawn de meshes statiques, \texttt{ADungeonGenerator}
expose des slots \texttt{TSubclassOf<AActor>} qui transforment le
dungeon en mini-game scaffold prêt à l'emploi :

\begin{itemize}
    \item \texttt{PlayerStartClass} : placé sur les 4 cellules walkable les plus proches du centre.
    \item \texttt{CornerSpawnerClass} (NPC spawners) : scattered sur \texttt{NumCornerSpawners} cellules walkable aléatoires.
    \item \texttt{RandomPickupClass} : scattered sur \texttt{NumRandomPickups} cellules walkable aléatoires.
    \item L'allocation garantit qu'aucune cellule n'héberge deux catégories d'actors simultanément.
    \item Z-offsets séparés (\texttt{GameplayActorZOffsetCm}, \texttt{PlayerStartZOffsetCm}) pour positionner correctement les capsules \texttt{APlayerStart} (qui descendent ~88 cm sous leur origine) au-dessus du sol.
\end{itemize}
...
\caption{Détail d'une intersection de couloirs : les meshes de sol
sont choisis aléatoirement parmi
\texttt{FloorMeshVariants} (variation visuelle), les walls sont
orientés selon leurs voisins walkable. La connectivité globale
walkable a été validée par flood-fill BFS avant spawn.}
...
\caption{Vue d'une pièce avec un \texttt{APlayerStart} (rendu par sa
capsule), un NPC spawner et plusieurs pickups. L'allocation
non-collisionnelle garantit qu'aucune cellule walkable n'héberge
deux catégories d'actors. Le \texttt{ANavMeshBoundsVolume} couvre
toute la grille, permettant l'IA NPC de naviguer.}
