...
\epigraph{\textit{``The best way to predict the future is to invent it.''}}{--- Alan Kay}

Ce chapitre analyse les limites rencontrées lors du développement, les leçons apprises, et les pistes d'amélioration futures. L'évolution itérative du projet illustre que l'optimisation HPC est un processus d'exploration où certaines intuitions se révèlent fructueuses et d'autres contre-productives.
...
% =============================================================================
\section{Leçons HPC retenues}
\label{sec:limites:lecons}
% =============================================================================

\begin{important}{Les 5 leçons principales}
\begin{enumerate}
    \item \textbf{Profiler avant d'optimiser}. Notre première intuition (« la propagation domine ») était fausse. La sélection min-entropie domine 8× la propagation.
    \item \textbf{Une seule région \texttt{parallel}} pour la propagation. Le coût fork/join × N niveaux BFS est catastrophique sinon.
    \item \textbf{A/B testing avec médiane}, pas moyenne. Sur 5 optims source-level CSAPP, une seule (LTO) a survécu. Le compilateur en \texttt{-O3 -march=native} fait déjà le travail.
    \item \textbf{Le GPU n'est pas une solution magique}. Sur ce workload, GH200 est plus lent que CPU EPYC. Le data-parallelism massif requiert un re-design algorithmique (batch).
    \item \textbf{Déterminisme et parallélisme} ne sont pas incompatibles, mais demandent une discipline : jitter stateless, réduction ordonnée, atomics commutatives. Vérification systématique par diff bit-à-bit.
\end{enumerate}
\end{important}
