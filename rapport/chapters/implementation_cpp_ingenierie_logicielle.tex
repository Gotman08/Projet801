...
Voir \texttt{schemas/wave\_memory\_layout.md} pour le prompt de génération.\\
Layout flat de la wave : un seul buffer \texttt{vector<u64>}, indexation \\
\texttt{(r, c)} → offset \texttt{(r * cols + c) * words\_per\_cell}, BitsetView non-propriétaire.
}}
...
for (std::ptrdiff_t c = 0; c < total_cells; ++c) {
    ...
    for (std::size_t w = 0; w + 1 < words_per_cell_; ++w) {
        words_[base + w] = ~0ULL;  // tous bits set
    }
