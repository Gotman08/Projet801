...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{results/figures/schemas/overlap_compatibility.png}
...
\midrule
Sortie utilise uniquement des tuiles du sample & \texttt{test\_solver} \\
...
Déterminisme : même seed $\to$ même output (bit-à-bit) & \texttt{test\_solver*} \\
Bit-identité serial = OMP = Kokkos pour $\{1, 2, 4, 8\}$ threads & \texttt{test\_solver\_*} \\
Reseed produit une sortie différente (haute probabilité) & \texttt{test\_solver\_*} \\
\bottomrule
\end{tabularx}
