...
\epigraph{\textit{``Performance portability without sacrificing performance.''}}{--- Kokkos design goal}
...
Kokkos abstrait ces différences. Le développeur écrit un \textbf{kernel parallèle} (\texttt{Kokkos::parallel\_for}) avec des \textbf{vues} (\texttt{Kokkos::View<T*>}) ; le compilateur Kokkos génère le code adapté au backend cible.
...
Voir \texttt{schemas/kokkos\_architecture.md} pour le prompt de génération.\\
Pipeline Kokkos : Wave \texttt{vector<u64>} → \texttt{Kokkos::View} (HostSpace ou CudaSpace) →
\texttt{parallel\_for} avec \texttt{PropagateOp} fonctor → atomic ops sur le device → \texttt{deep\_copy} retour si GPU.
}}
...
\midrule
Counter atomique pour next\_frontier & \texttt{Kokkos::atomic\_fetch\_add(\&next\_count(0), 1)} \\
\bottomrule
\end{tabularx}
...
Une optimisation future serait de mirrorer le wave une fois au début du solve, et faire min-entropy sur device avec un kernel séparé. Documenté comme piste future dans \texttt{docs/CHOICES.md}.
...
Sur la partition \texttt{armgpu} de Romeo (NVIDIA GH200, sm\_90), le script \texttt{scripts/build\_kokkos\_gpu\_romeo.slurm} effectue :

\begin{lstlisting}[language=bash]
spack load gcc@11.4.1 cmake ninja cuda@12.9
...
Ces pistes sont documentées au Chapitre~\ref{chap:limites}. Le port reste un livrable utile : le code compile et tourne sur GPU, les tests d'invariants passent, mais pour le speedup pratique, le CPU EPYC 9654 reste le meilleur choix sur ce workload.
