...
La sélection min-entropie domine 8× la propagation BFS. Conséquence directe : nous parallélisons les deux étapes, mais l'effort principal porte sur la sélection.
...
Pour éviter qu'une cellule soit ajoutée plusieurs fois au frontier suivant (touchée par plusieurs threads voisins en même temps), on utilise un drapeau \texttt{in\_queue[cell]} et un \texttt{compare\_exchange} :
...
\midrule
\texttt{-O3 -march=native} (baseline) & 4.280 s & --- \\
+ \texttt{USE\_LTO=ON} & 4.134 s & \textbf{+3.4\% gardée} \\
+ LTO + PGO & 4.143 s & +0.4\% rejetée \\
...
\bottomrule
\end{tabularx}
\caption{A/B testing source-level. Le compilateur \texttt{-O3 -march=native} fait déjà la plupart des optims CSAPP.}
\label{tab:omp:rejected}
\end{table}

Sur les 5 candidats source-level testés (issus de CSAPP), un seul (LTO) a survécu : le compilateur en \texttt{-O3 -march=native} effectue déjà la plupart de ces transformations.
...
La réduction finale parcourt \texttt{partials[]} en ordre de chunk
croissant. On évite la clause \texttt{reduction(min:...)} d'OMP qui
serait non-déterministe (dépendante de l'ordre d'arrivée des threads).
