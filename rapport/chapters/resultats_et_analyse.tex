...
\epigraph{\textit{``If you cannot measure it, you cannot improve it.''}}{--- Lord Kelvin}
...
\begin{figure}[H]
\centering
\schemaImage{Decomposition du temps WFC serial 128x128}
...
\caption{Speedup binary\_L11 Romeo. Plus la grille est grande, plus le speedup peak est élevé.}
...
\caption{Bande de speedup binary\_L11 (min/max sur 3 répétitions). La
zone ombrée encadre la dispersion expérimentale ; on lit directement
sur le graphe les régions stables (1-8 threads) et instables
(>32 threads, où la régression varie d'un run à l'autre).}
...
\caption{Efficacité parallèle $E(p) = S(p)/p$ sur binary\_L11. À 16
threads sur 256×256, $E \approx 0.51$ (51\% d'efficacité). Au-delà
de 32 threads, l'efficacité s'effondre sous 0.1 quelle que soit la
taille.}
...
\caption{Throughput (Mcells/s) binary\_L11. Le throughput maximal est
atteint à 16 threads sur 256×256 (\textbf{15.4 Mcells/s}), $14\times$
plus haut que le throughput serial sur la même taille (1.07 Mcells/s).}
...
\caption{Heatmap speedup binary\_L11 (taille × threads). La diagonale
verte (sweet spot) montre clairement que la taille optimale de grille
suit le nombre de threads : pour 192 cores, il faudrait probablement
une grille bien plus grande que 256×256 pour amortir la fork/join.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{results/figures/amdahl/amdahl_binary_L11_256.png}
\caption{Fit Amdahl binary\_L11 256×256, $f = 0.944$, ceiling
théorique $17.8\times$, peak observé $8.23\times$ (46\% du ceiling).}
\label{fig:amdahl_binary_256}
\end{figure}
...
\caption{Évolution du fit Amdahl binary\_L11 avec la taille de grille.
La fraction parallélisable $f$ croît rapidement de 0.18 (32×32) à 0.94
(256×256) : plus la grille est grande, plus la propagation et la
sélection min-entropie deviennent dominantes face aux coûts fixes.}
...
\caption{Fit Amdahl binary\_L11 128×128 \textbf{après} activation de
l'optim frontier-threshold. $f$ passe de 0.928 à 0.956, ceiling de
$13.8\times$ à $22.9\times$. Le peak observé monte de $5.27\times$ à
$6.10\times$ : l'optim ne suffit pas à atteindre le nouveau ceiling
(coûts supra-Amdahl), mais elle pousse le mur.}
...
\caption{Fits Amdahl par sample × taille. terrain\_L33 suit le même
pattern que binary\_L11 (croissance de $f$ avec la taille). smooth\_N3
plafonne à $f = 0.74$ même à 128×128 : peu de tuiles ($L=12$) et
$N=3$ → parallélisme structurel limité, indépendamment de la grille.}
...
\caption{terrain\_L33 scale mieux que binary\_L11 à taille égale.}
...
\caption{terrain\_L33 : efficacité et throughput. À 128×128 et 8
threads, $E \approx 71\%$ — meilleur que binary\_L11 à taille
identique grâce au travail par cellule plus dense ($L = 33$ bits à
manipuler vs 11).}
...
\caption{Heatmap terrain\_L33. Le sweet spot diagonal est plus
prononcé que pour binary\_L11 : la pente d'optimum entre taille et
threads est plus douce.}
...
\caption{Bande min/max terrain\_L33. La dispersion expérimentale
reste basse jusqu'à 16 threads (régime stable), puis explose au-delà
(la régression varie d'une exécution à l'autre).}
...
\caption{smooth\_N3 scale beaucoup moins bien : peu de tuiles + $N = 3$ = peu de parallélisme par cellule.}
...
\caption{Efficacité parallèle smooth\_N3.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
\centering
...
\caption{smooth\_N3 : efficacité et throughput plafonnent rapidement.
Le plateau visible à 4 threads sur l'efficacité confirme l'analyse :
au-delà, les barrières BFS dominent.}
...
\caption{Heatmap smooth\_N3. Sweet spot très étroit autour de
4 threads, quel que soit la taille — comportement borné par la
structure du sample, pas par la grille.}
...
\caption{Bande min/max smooth\_N3. La bande s'élargit tôt (dès
8 threads) : la variance entre runs croît rapidement quand on dépasse
le sweet spot.}
...
\caption{Comparaison directe serial / OMP / Kokkos sur binary\_L11
pour 4 tailles. Kokkos suit OMP de loin sur les petites grilles
(overhead Views + dispatch) et se rapproche de OMP à 256×256 où le
travail amortit l'overhead.}
...
\caption{Backends sur \texttt{terrain\_L33} et \texttt{smooth\_N3}, 3
tailles. Sur smooth\_N3, Kokkos suit serial très près à toutes les
tailles : peu de parallélisme exploitable. Sur terrain\_L33, Kokkos
bénéficie davantage de la parallélisation à mesure que la grille
grandit.}
...
\caption{binary\_L11 128×128 \textbf{après} optim frontier-threshold.
Le delta entre Kokkos et OMP se réduit visiblement sous le régime
optimisé : l'optim profite aux deux backends, mais OMP en absorbe
mieux le gain à haut thread count.}
...
\begin{tabular}{lrl}
\toprule
\textbf{Optim} & \textbf{Median 128×128} & \textbf{Verdict} \\
\midrule
Cache \texttt{f * log(f)} thread\_local & 4.749 s & -6.7\%, \textbf{rejetée} \\
\texttt{std::queue} → vector FIFO & 4.586 s & -3\%, dans le bruit \\
Hoist \texttt{wave.at(c)} (CSE manuel) & 4.342 s & 0\%, variance \\
LTO + PGO (en plus de LTO) & 4.143 s & +0.4\% supplémentaire, complexité non justifiée \\
Work-density gate dans \texttt{propagate\_tasks} & non mesuré & rollback (risque de régression sur binary) \\
\bottomrule
\end{tabular}
\caption{Optimisations rejetées en A/B testing. Le compilateur \texttt{-O3 -march=native} fait déjà la plupart des optims source-level.}
...
\caption{Sample \texttt{multivalue\_terrain} (eau/sable/herbe/roche,
33 tuiles à $N=2$). À 128×128, plusieurs îles indépendantes
apparaissent : la zone est plus grande que dans l'échantillon, et le
solveur reproduit uniquement les transitions locales
(eau→sable→herbe→roche), il ne sait pas qu'il doit faire une seule île.}
...
\caption{Sample \texttt{multivalue\_smooth} (4 valeurs en gradients
lisses). Augmenter $N$ (2 → 3) capture des motifs locaux plus précis :
la sortie reproduit mieux les courbes de transition, au prix d'un
tile set plus gros et d'un solveur plus lent (12 tuiles à $N=2$
vs. 73 à $N=3$).}
...
% =============================================================================
\section{Synthèse : lecture transversale des figures}
\label{sec:res:synthese}
% =============================================================================

Cette section relie les figures précédentes au code source qui produit
les gains observés, et discute les optimisations qui feraient \emph{mieux
scaler} le solveur au-delà du peak actuel.

\subsection{Pourquoi le speedup peak vit à 16 threads}

Trois figures lues conjointement expliquent le peak à 16 threads sur
256×256 :

\begin{itemize}
    \item Figure~\ref{fig:speedup_binary} (speedup) : la courbe monte
    jusqu'à 16 threads puis chute brutalement.
    \item Figure~\ref{fig:efficiency_binary} (efficacité) : à 16
    threads sur 256×256, $E = 0.51$. Au-delà, $E$ décroche.
    \item Figure~\ref{fig:amdahl_binary_256} (Amdahl) : le fit donne
    $f = 0.944$, ceiling théorique $17.8\times$ ; le peak observé
    n'atteint que 46\% du ceiling.
\end{itemize}

Le \emph{gap} entre ceiling et peak est imputable à des coûts
\textit{supra-Amdahl} : la fork/join, la contention atomique, les
frontières BFS courtes. Aucun de ces coûts n'est modélisé par Amdahl
(qui suppose un parallélisme parfait sur la fraction $f$). C'est
exactement le rôle des optims OMP gardées
(Section~\ref{sec:res:synthesis}) : amincir l'écart entre la courbe
mesurée et l'asymptote $f$.

\subsection{Lecture du heatmap : où vit le sweet spot}

Le heatmap binary\_L11 (Figure~\ref{fig:heatmap_binary}) trace
\textbf{taille de grille × threads} avec la couleur = temps. La
diagonale verte (sweet spot) montre que la taille optimale de grille
\emph{suit} le nombre de threads. Lecture pratique :

\begin{itemize}
    \item À 8 threads, optimum vers 128×128.
    \item À 16 threads, optimum vers 256×256.
    \item À 32+ threads, on aurait besoin de grilles 512×512 ou plus
    (qui n'ont pas été mesurées : les runs 4h timeout).
\end{itemize}

Cette information dimensionne directement les choix d'utilisation
(Section~\ref{sec:res:recommandations}).

\subsection{Pourquoi smooth\_N3 plafonne}

Le triplet figures \ref{fig:speedup_smooth},
\ref{fig:scaling_smooth_combo} et \ref{fig:amdahl_other_samples}d
montre la limite \emph{structurelle} de smooth\_N3 :

\begin{itemize}
    \item Speedup peak à 2.23× à 4 threads.
    \item Efficacité chute sous 0.5 dès 8 threads.
    \item Amdahl $f = 0.74$ même à 128×128 (vs 0.94 pour binary\_L11).
\end{itemize}

La cause est dans l'algorithme de propagation, pas dans son
implémentation. Citons le calcul du chunk dans
\texttt{propagate\_tasks} :
...
Quand \texttt{frontier\_size} reste $< $ \texttt{kSerialFallback} (cas
smooth\_N3 avec ses motifs lisses qui propagent peu), on retombe en
série : pas d'opportunité de gain. Le frontier-threshold
(Figure~\ref{fig:optim_frontier}) ne peut pas réparer un manque de
parallélisme structurel.

\subsection{Comment le code OMP atteint son ceiling}

Trois optimisations expliquent l'essentiel du gain visible sur les
courbes \texttt{omp} (Figures~\ref{fig:backends_binary},
\ref{fig:backends_other}) :

\paragraph{1. Une seule région \texttt{parallel}.} Le squelette de
\texttt{propagate\_tasks} (voir Chapitre~\ref{chap:openmp},
Section~\ref{sec:omp:propagate}) ouvre \emph{une seule} région
parallel et pilote les niveaux BFS via \texttt{\#pragma omp single}.
Sans cela, on paye 50 µs × 8000 niveaux $\approx 400$ ms de pur
fork/join — soit plus que le travail utile total.

\paragraph{2. Atomic relaxed-load skip.} Dans
\texttt{atomic\_and\_with} (Section~\ref{sec:omp:propagate}), une
lecture relaxed précède le \texttt{lock and} pour éviter l'opération
quand l'AND serait un no-op. Sur EPYC, le \texttt{lock and} déclenche
un broadcast d'invalidation cross-NUMA (1-3 µs) ; la lecture relaxed
ne nécessite que la ligne en \textit{shared} state. Mesuré
$\sim 50\%$ de trafic atomique en moins.

\paragraph{3. Per-thread frontier alignas(64).} Chaque thread possède
son \texttt{ThreadFrontier} aligné sur ligne de cache. Pas de faux
partage entre threads, pas de lock global pour pousser dans la
frontière suivante (concaténée par le \texttt{single} en fin de
niveau).

\subsection{Ce que les figures ne montrent pas : les optims rejetées}

Le tableau~\ref{tab:res:rejected} liste 5 optims source-level
\emph{rejetées} après mesure A/B. C'est aussi instructif que les
optims gardées :

\begin{itemize}
    \item Cache thread\_local de \texttt{f * log(f)} : -6.7\%
    (régression nette). Le cache miss de la table déclasse le calcul
    flottant inline qu'il était censé éviter.
    \item \texttt{std::queue} → vector FIFO : 0\% net (dans la
    variance). Le vector réduit l'allocation mais ne change rien à
    l'algorithme.
    \item LTO + PGO : +0.4\% supplémentaire au-dessus de LTO seul.
    Pas justifié vu la complexité de la chaîne PGO.
\end{itemize}

\subsection{Optims qui feraient mieux scaler}
\label{sec:res:scale_better}

Trois optimisations \emph{non implémentées} adresseraient les coûts
supra-Amdahl visibles dans les figures actuelles. Elles sont
documentées en détail au Chapitre~\ref{chap:limites} ; voici leur
impact attendu sur les figures de scaling.

\paragraph{Heuristique d'entropie incrémentale.} La sélection
min-entropie domine $\sim 85\%$ du temps série
(Figure~\ref{fig:profile_breakdown}). Maintenir une priority queue
indexée par cellule, mise à jour incrémentale à chaque collapse,
ferait passer la sélection de $O(r \cdot c \cdot L)$ par collapse à
$O(\log(r \cdot c))$ amorti. Pour 256×256 (~65k cellules), facteur
$\sim 1000$ sur la phase qui domine.

\textbf{Effet attendu sur les figures} : le serial baseline tomberait
de 61 s à $\sim 1$ s sur 256×256 ; tous les speedups deviendraient
plus modestes (la fraction parallélisable $f$ chuterait), mais les
\emph{temps absolus} seraient drastiquement plus bas. Le throughput
peak passerait de 15 Mcells/s à plusieurs centaines.

\paragraph{NUMA-aware partitioning.} La cause (3) de la régression
high-thread est la contention atomique \emph{cross-NUMA} (cache-line
ping-pong, 3× plus cher qu'intra-NUMA sur EPYC 8 NUMA nodes).
Partitionner la wave par socket et propager les frontières
inter-partitions via boîtes aux lettres explicites (au lieu d'atomics
sur mémoire partagée) éliminerait ce coût.

\textbf{Effet attendu} : la régression au-delà de 32 threads
s'aplatirait, le peak speedup remonterait probablement à
$12$-$14\times$ à 64 threads (vs $1.26\times$ actuel).

\paragraph{Batch GPU.} Le GPU GH200 est plus lent que le CPU
(Figure~\ref{fig:backends_binary}) parce qu'on lui donne un seul
solveur sur une grosse grille. Lancer K solveurs indépendants sur K
petites grilles, chacun assigné à un SM, exploiterait le
data-parallelism massif du GPU.

\textbf{Effet attendu} : 32 attempts sur GH200 en parallèle
prendraient le temps d'un seul attempt, soit $\sim 2$ s pour 32
solveurs 64×64 (vs $\sim 64$ s en série). Le GPU deviendrait
intéressant pour les workloads de génération en batch (ex. game
open-world avec K régions à générer).

\begin{recommendation}
Si on devait pousser ce projet plus loin, l'ordre de priorité serait :
\begin{enumerate}
    \item \textbf{Entropie incrémentale} (PQ ou tournament tree) :
    meilleur ratio bénéfice/risque, attaque la phase dominante.
    \item \textbf{Batch GPU} : seule voie réaliste pour rentabiliser
    le port Kokkos/CUDA déjà en place.
    \item \textbf{NUMA partitioning} : refactor lourd, gain visible
    seulement à très haut thread count (utile sur Romeo, marginal
    ailleurs).
\end{enumerate}
\end{recommendation}
