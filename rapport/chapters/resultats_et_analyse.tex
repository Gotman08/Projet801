...
\caption{terrain\_L33 128×128, $f = 0.94$.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{results/figures/amdahl/amdahl_smooth_N3_128.png}
\caption{smooth\_N3 128×128, $f = 0.74$ (cas limite).}
\end{subfigure}

\caption{Fits Amdahl par sample. terrain\_L33 suit le même pattern que
binary\_L11 (croissance de $f$ avec la taille). smooth\_N3 plafonne à
$f = 0.74$ même à 128×128 : peu de tuiles ($L=12$) et $N=3$ →
parallélisme structurel limité.}
...
\caption{terrain\_L33 128×128.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{results/figures/scaling/backends_smooth_N3_128.png}
\caption{smooth\_N3 128×128.}
\end{subfigure}

\caption{Backends sur autres samples 128×128. Sur smooth\_N3,
Kokkos suit serial très près : peu de parallélisme exploitable. Sur
terrain\_L33, Kokkos bénéficie davantage de la parallélisation.}
...
par le solveur. Trois seeds différents par sample montrent la variété
sans dégrader la cohérence locale (chaque sous-bloc $N \times N$
apparaît bien dans l'échantillon source).

\subsection{rooms (dungeon binaire)}
...
\caption{Sample \texttt{rooms} (binaire, $N=2$, 11 tuiles), 3 seeds.
Pièces rectangulaires connectées par couloirs ; la composante
walkable est connexe (vérifiée par flood-fill BFS de \texttt{wfc\_dungeon}).}
\label{fig:gallery_rooms}
\end{figure}

\subsection{skyline (urbain multi-valeurs)}
...
\caption{Sample \texttt{skyline} (4 valeurs, $N=3$, 79 tuiles), 3 seeds.
Bâtiments cohérents avec fenêtres et fondations, ciel uniforme. Les
trois variantes diffèrent dans la disposition mais respectent la
grammaire locale du sample d'entrée.}
\label{fig:gallery_skyline}
\end{figure}

\subsection{plant (plante avec fleurs)}
...
\caption{Sample \texttt{plant} (4 valeurs, $N=3$, 94 tuiles), 3 seeds.
Tiges, fleurs et feuilles cohérentes, ancrées au sol. Cas riche
($L=94$) où l'expansion D4 (\texttt{--symmetries 4 ou 8}, voir
Chapitre~\ref{chap:extensions}) permettrait une variation orientation
plus marquée.}
